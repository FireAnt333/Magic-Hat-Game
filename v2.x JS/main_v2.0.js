...
- Game now functions using the HTML canvas element to display a menu and either colored squares or 16x16 sprites for tiles and the player, depending on whether the imageDisplay variable is set to 'colors' or 'sprites'. 
- Sprites are blurry due to the inherent canvas upscaling issue. A fix will be attempted in version 2.1. 
...
let gridSize = 10;
const gridIndexPx = canvasSize/gridSize;
//let fieldHeight = 10; 
//let fieldWidth = 10;
    ...
    for (let j = 0; j < gridSize; j++) {
      if (imageDisplay === 'colors') {
        if (displayField[i][j] === fieldCharacter) {
          ...
          ctx.fillStyle = 'pink'; // Error indicator; tile is none of the ones that should show up in the game
        }
        ctx.fillRect((j)*(gridIndexPx) + 3, (i)*(gridIndexPx) + 3, (gridIndexPx) - 5, (gridIndexPx) - 5);
      } 
      
      else if (imageDisplay === 'sprites') {
        if (displayField[i][j] === fieldCharacter) {
          // Draw a field image
          ctx.drawImage(imgField, j*gridIndexPx, i*gridIndexPx, gridIndexPx, gridIndexPx);
        } else if (displayField[i][j] === hiddenTileCharacter1 || displayField[i][j] === hiddenTileCharacter2) {
          // Draw a black box
          ctx.fillStyle = 'black';
          ctx.fillRect(j*gridIndexPx, i*gridIndexPx, gridIndexPx, gridIndexPx);
        } else if (displayField[i][j] === hole) {
          // Draw a field image, then a hole image over it
          ctx.drawImage(imgField, j*gridIndexPx, i*gridIndexPx, gridIndexPx, gridIndexPx);
          ctx.drawImage(imgHole, j*gridIndexPx, i*gridIndexPx, gridIndexPx, gridIndexPx);
        } else if (displayField[i][j] === hat) {
          // Draw a field image, then a hat image over it
          ctx.drawImage(imgField, j*gridIndexPx, i*gridIndexPx, gridIndexPx, gridIndexPx);
          ctx.drawImage(imgHat, j*gridIndexPx, i*gridIndexPx, gridIndexPx, gridIndexPx);
        } else if (displayField[i][j] === playerCharacter) {
          // Draw a field image, then a wizard image over it
          ctx.drawImage(imgField, j*gridIndexPx, i*gridIndexPx, gridIndexPx, gridIndexPx);
          ctx.drawImage(imgWizard, j*gridIndexPx, i*gridIndexPx, gridIndexPx, gridIndexPx);
        } else {
          ctx.fillStyle = 'pink'; // Error indicator; tile is none of the ones that should show up in the game
          ctx.fillRect((j)*(gridIndexPx) + 3, (i)*(gridIndexPx) + 3, (gridIndexPx) - 5, (gridIndexPx) - 5);
        }
...
function loadScene(scene) {
  ...
  if (scene === 'menu') {
    ...
    ctx.lineWidth = 5;
    ctx.fillStyle = 'white';
    ctx.fillRect(canvasSize*0.1, canvasSize*0.1, canvasSize*0.8, canvasSize*0.8);
    
    ctx.strokeStyle = 'blue';
    ctx.strokeRect(canvasSize*0.1, canvasSize*0.1, canvasSize*0.8, canvasSize*0.8);
    ctx.strokeStyle = 'yellow';
    ctx.strokeRect(canvasSize*0.11, canvasSize*0.11, canvasSize*0.78, canvasSize*0.78);
    ctx.strokeStyle = 'blue';
    ctx.strokeRect(canvasSize*0.12, canvasSize*0.12, canvasSize*0.76, canvasSize*0.76);
    
    ctx.fillStyle = 'black';
    ctx.font = '36px Helvetica';
    ctx.textAlign = 'center';
    ctx.fillText('Magic Hat', canvasSize/2, canvasSize*0.2);
    
    ctx.font = '24px Helvetica';
    ctx.fillText('Press "g" for a new game!', canvasSize/2, canvasSize*0.3);
    ctx.fillText('[Manual settings coming soon]', canvasSize/2, canvasSize*0.6);

    if (imageDisplay === 'sprites') {
      ctx.drawImage(imgWizard, canvasSize*0.45, canvasSize*0.45, canvasSize*0.1, canvasSize*0.1);
    }
    ...
    //ctx.fillRect(0, 0, canvasSize, canvasSize);
    //ctx.fillStyle = 'black';
    //ctx.fillText('Game scene placeholder', canvasSize*0.4, canvasSize*0.4);
    ...
    ctx.fillStyle = 'white'
    ctx.fillRect(canvasSize*0.15, canvasSize*0.3, canvasSize*0.7, canvasSize*0.4);

    ctx.strokeStyle = 'blue'
    ctx.strokeRect(canvasSize*0.15, canvasSize*0.3, canvasSize*0.7, canvasSize*0.4);
    ctx.strokeStyle = 'yellow'
    ctx.strokeRect(canvasSize*0.16, canvasSize*0.31, canvasSize*0.68, canvasSize*0.38);
    ctx.strokeStyle = 'blue'
    ctx.strokeRect(canvasSize*0.17, canvasSize*0.32, canvasSize*0.66, canvasSize*0.36);
    ...
    if (endGameReason === 'hat') {
      ctx.font = '24px Helvetica';
      ctx.fillText('Yay! You found the hat!', canvasSize/2, canvasSize*0.45);
    } else if (endGameReason === 'hole') {
      ctx.font = '24px Helvetica';
      ctx.fillText('Oh no! You fell into a hole!', canvasSize/2, canvasSize*0.45);
    } else {
      ctx.font = '24px Helvetica';
      ctx.fillText('[endGameReason not determined]', canvasSize/2, canvasSize*0.45);
    }

    ctx.font = '12px Helvetica';
    ctx.fillText('Press "m" to return to the menu', canvasSize/2, canvasSize*0.60);
...
function findPath(field) {
  ...
  while (timeOut < timeOutLimit) {
    ...
    let closestValidMoveDistance = canvasSize * 2;
