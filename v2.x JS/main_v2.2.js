...
- The solution path and field are now accessible when the game is active by pressing the "F" key control.
...
function refreshGrid() {
  ...
  let proxyField = [];
  if (fieldToShow === 'displayField') {
    updateDisplayField();
    proxyField = copy2DArray(displayField);
  } else if (fieldToShow === 'solutionField') {
    proxyField = copy2DArray(solutionField);
  }
  ...
  for (let i = 0; i < gridSize; i++) {
    for (let j = 0; j < gridSize; j++) {
      if (imageDisplay === 'colors') {
        if (proxyField[i][j] === fieldCharacter) {
          ic.fillStyle = fieldColor;
        } else if (proxyField[i][j] === hiddenTileCharacter1 || proxyField[i][j] === hiddenTileCharacter2) {
          ic.fillStyle = hiddenTileColor;
        } else if (proxyField[i][j] === hole) {
          ic.fillStyle = holeColor;
        } else if (proxyField[i][j] === pathCharacter) {
          ic.fillStyle = pathColor;
        } else if (proxyField[i][j] === hat) {
          ic.fillStyle = hatColor;
        } else if (proxyField[i][j] === playerCharacter) {
        ...
        if (proxyField[i][j] === fieldCharacter) {
          ...
          // Draw a field image, then a path indicator over it
          ic.drawImage(imgField, j*gridIndexPx, i*gridIndexPx, gridIndexPx, gridIndexPx);
          ic.fillStyle = pathColor;
          ic.fillRect((j*gridIndexPx + gridIndexPx*0.25), (i*gridIndexPx + gridIndexPx*0.25), gridIndexPx*0.5, gridIndexPx*0.5);
            if (checkMove([i-1, j]) !== 'boundary') {
              if (proxyField[i-1][j] === pathCharacter || proxyField[i-1][j] === playerCharacter || proxyField[i-1][j] === hat) {
                // Draw yellow above
                ic.fillRect((j*gridIndexPx + gridIndexPx*0.25), i*gridIndexPx, gridIndexPx*0.5, gridIndexPx*0.25);
              }
            }
            if (checkMove([i, j+1]) !== 'boundary') {
              if (proxyField[i][j+1] === pathCharacter || proxyField[i][j+1] === playerCharacter || proxyField[i][j+1] === hat) {
                // Draw yellow to the right
                ic.fillRect((j*gridIndexPx + gridIndexPx*0.75), (i*gridIndexPx + gridIndexPx*0.25), gridIndexPx*0.25, gridIndexPx*0.5);
              }
            }
            if (checkMove([i+1, j]) !== 'boundary') {
              if (proxyField[i+1][j] === pathCharacter || proxyField[i+1][j] === playerCharacter || proxyField[i+1][j] === hat) {
                // Draw yellow below
                ic.fillRect((j*gridIndexPx + gridIndexPx*0.25), (i*gridIndexPx + gridIndexPx*0.75), gridIndexPx*0.5, gridIndexPx*0.25);
              }
            }
            if (checkMove([i, j-1]) !== 'boundary') {
              if (proxyField[i][j-1] === pathCharacter || proxyField[i][j-1] === playerCharacter || proxyField[i][j-1] === hat) {
                // Draw yellow to the left
                ic.fillRect(j*gridIndexPx, (i*gridIndexPx + gridIndexPx*0.25), gridIndexPx*0.25, gridIndexPx*0.5);
              }
            }
...
function promptMessage(message) {
  ...
  if (message === 'clear') {
    ...
    // Make sure there's a listener set up to response to this question
...
document.addEventListener('keydown', event => {
  ...
  if (currentScene === 'menu') { // MENU: Listens for input in the menu
  ...
  } else if (currentScene === 'game' && fieldToShow === 'displayField') { // GAME/PLAY: Listens for input if the displayField is being displayed
  ...
  } else if (currentScene === 'game' && fieldToShow === 'solutionField') { // GAME/SOLUTION: Listens for input if the solutionField is being displayed
      switch (event.keyCode) {
        case 71: // G / Return to game
          fieldToShow = 'displayField';
          refreshGrid();
          break;
        case 77: // M / Return to menu
          fieldToShow = 'displayField';
          loadScene('menu');
          break;
      }

  } else if (currentScene === 'endGame') { // ENDGAME: Listens for input on the end-game card
